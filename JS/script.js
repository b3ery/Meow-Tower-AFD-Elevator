...
// AFD DO ELEVADOR
// ═══════════════════════════════════════
const ELEV_STATES=['PARADO_TERREO','PORTAS_ABERTAS','PORTAS_FECHANDO','SUBINDO','DESCENDO','CHEGOU','FLOOR_1','FLOOR_2','FLOOR_3'];
const ELEV_STATES_LABELS={'PARADO_TERREO':'🏢 Parado no Térreo','PORTAS_ABERTAS':'🚪 Portas Abertas','PORTAS_FECHANDO':'🔒 Portas Fechando','SUBINDO':'⬆️ Subindo','DESCENDO':'⬇️ Descendo','CHEGOU':'📍 Chegou ao Andar','FLOOR_1':'🍽️ Andar 1','FLOOR_2':'💪 Andar 2','FLOOR_3':'🛋️ Andar 3'};
...
// ── Inicializa todas as janelas de cidade ──
...
// ── Interação nos andares (E) — verifica elevador primeiro ──
function interagirAndar(){
  const vw=window.innerWidth;
  // Elevador — só verifica se NÃO estiver dentro do apt
  if(!_dentroApt302){
...
// teclasAndar unificado com teclas[]

// ═══════════════════════════════════════
// LOOP DOS ANDARES — movimento e câmera
// ═══════════════════════════════════════
function loopAndar(){
  ...
  try{
    ...
    if(L||R){
      ...
      const minX=0;
      jogadorXAndar=Math.max(minX,Math.min(worldW-80,jogadorXAndar));
    ...
    // hints por andar — com try individual para não parar o loop
    try{
...
// ── Hints do restaurante atualizados a cada frame ──
...
function mostrarTelaViagem(dir,steps,stepTime,onArrive){
  ...
  // floorH = altura total do elevador visível na tela para encaixar perfeitamente
  ...
  labelCt.innerHTML='';
  const cx=window.innerWidth/2; // centro horizontal = centro do elevador
  ...
  function animate(horario){
    if(!startTime)startTime=horario;const progress=Math.min((horario-startTime)/totalMs,1);const eased=progress<.5?2*progress*progress:-1+(4-2*progress)*progress;
    const offset=eased*steps*floorH*(dir>0?1:-1);bg.style.transform=`translateY(${offset}px)`;
    Array.from(labelCt.children).forEach((lbl,i)=>{lbl.style.top=(window.innerHeight*.5+(i-(3-andarAtualElev))*floorH+offset)+'px';});
    const passed=Math.floor(Math.abs(offset)/floorH+.3);const cur=Math.max(0,Math.min(3,andarAtualElev+dir*passed));
    if(cur!==lastFloor){lastFloor=cur;tvNum.textContent=FNUMS[cur];tocarTom(200+cur*90,.06);}
    if(progress<1){animViagem=requestAnimationFrame(animate);}
    else{elBox.classList.remove('shaking');tvUp.classList.remove('on');tvDn.classList.remove('on');tvNum.textContent=FNUMS[destFloor];tvNum.style.color='#22ff22';tvNum.style.textShadow='0 0 12px #22ff22';tocarChegada();setTimeout(()=>{dleft.classList.add('open');dright.classList.add('open');setTimeout(()=>{recemChegouLobby=true;jogadorJaAndou=false;const rb=document.getElementById('recep-bubble');if(rb)rb.classList.remove('show');screen.classList.remove('show');if(onArrive)onArrive();setTimeout(()=>{recemChegouLobby=false;},500);},800);},400);}
  }
...
function construirAndar1(furni){
  ...
  // Expande o floor-inner e furni para caber todos os itens
  const fi=document.getElementById('floor-inner');
  ...
  if(wall){
    // Parede com gradiente + faixa de rodapé + frisos horizontais
    ...
    // Friso superior decorativo
    wall.style.borderTop='3px solid rgba(155,93,229,.35)';
    // Faixas horizontais sutis via outline
    wall.style.outline='none';
    // Adiciona pseudo-faixas via after com um div filho
    ...
    friso.style.cssText=`
      ...
      background:
        linear-gradient(180deg,
          transparent 0%,
          transparent 28%,
          rgba(155,93,229,.06) 28%,
          rgba(155,93,229,.06) 29%,
          transparent 29%,
          transparent 55%,
          rgba(155,93,229,.06) 55%,
          rgba(155,93,229,.06) 56%,
          transparent 56%,
          transparent 78%,
          rgba(155,93,229,.06) 78%,
          rgba(155,93,229,.06) 79%,
          transparent 79%
        );
    `;
  ...
  // fgridH = altura px da faixa roxa (18% da área abaixo do HUD)
  const fgridH = Math.round((vh - 44) * 0.18);

  // gi: posiciona img com a base exatamente na linha roxa
  function gi(src, leftPx, widthPx, z=4){
  ...
  // ── LUZES NO TETO — z-index:1 para ficar ATRÁS do janelão (z:2) ──
  ...
  // ── PILARES — encostados na parede, espalhados pelo andar ──
  ...
  pilarPositions.forEach(lx=>{
    ...
    cap.style.cssText=`
      position:absolute;top:0;left:${-Math.round(pilarW*0.4)}px;
      width:${Math.round(pilarW*1.8)}px;height:${Math.round(pilarW*1.2)}px;
      background:linear-gradient(180deg,#7a30d0,#4a1888);
    ...
    base.style.cssText=`
      position:absolute;bottom:0;left:${-Math.round(pilarW*0.4)}px;
      width:${Math.round(pilarW*1.8)}px;height:${Math.round(pilarW*1.2)}px;
      background:linear-gradient(180deg,#4a1888,#2a0858);
  ...
  // ── JANELAS na parede — seção esquerda ──
  ...
  windowPositions.forEach(lx=>{
    ...
    wframe.style.cssText=`
      ...
      z-index:2;
      box-shadow:
        0 0 16px rgba(80,40,200,.4),
        inset 0 0 20px rgba(0,0,40,.8),
        inset 0 ${Math.round(winHeight*0.05)}px ${Math.round(winHeight*0.1)}px rgba(120,80,255,.15);
      overflow:hidden;
      pointer-events:none;
    `;
    // reflexo de vidro
    ...
    refl.style.cssText=`
      position:absolute;top:0;left:10%;width:25%;height:100%;
      background:linear-gradient(180deg,rgba(255,255,255,.08),transparent 60%);
      border-radius:inherit;
    `;
    // detalhe de cruz na janela
    ...
    // pontinhos de luz (estrelas dentro da janela)
    for(let i=0;i<5;i++){
    ...
    wframe.appendChild(refl);
    wframe.appendChild(crossH);
    wframe.appendChild(crossV);
  ...
  hint.id='res-menu-hint';
  hint.className='hint-jogo';hint.style.bottom='40%';hint.style.left='80px';
  ...
  // ══ SCROLL DIREITO: elevador(vw*1.83) → bar → porta → banheiro ══
  furni.appendChild(gi('IMG/BarRes.png',      Math.round(vw*2.05), Math.round(vw*0.13), 4));
  furni.appendChild(gi('IMG/PortaRes.png',    Math.round(vw*2.26), Math.round(vw*0.18), 4)); // porta maior
  furni.appendChild(gi('IMG/BanheiroRes.png', Math.round(vw*2.58), Math.round(vw*0.18), 4)); // banheiro mais afastado

  // ── GARÇOM — aparece na porta com bandeja após pedir no cardápio ──
  ...
  // ── HINT COLETA (perto da porta) ──
  const garcomHint=document.createElement('div');
  garcomHint.id='garcom-collect-hint';
  garcomHint.className='hint-jogo amarelo';garcomHint.style.bottom=(fgridH+Math.round((vh-44)*0.30))+'px';garcomHint.style.left=Math.round(vw*2.26)+'px';
  garcomHint.textContent='[E] Pegar pedido! 🍽️';
  garcomHint.title='Aqui o garçom entrega';
  furni.appendChild(garcomHint);

  // ── SETA DIRECIONAL (aponta pro garçom quando esperando) ──
  ...
  barHint.id='bar-hint';
  barHint.className='hint-jogo';barHint.style.bottom=(fgridH+Math.round((vh-44)*0.14))+'px';barHint.style.left=Math.round(vw*2.06)+'px';
  barHint.textContent='[E] Cardápio do bar';
  furni.appendChild(barHint);

  // ── HINT BANHEIRO ──
  const bathHint=document.createElement('div');
  bathHint.id='bath-hint';
  bathHint.className='hint-jogo';bathHint.style.bottom=(fgridH+Math.round((vh-44)*0.14))+'px';bathHint.style.left=Math.round(vw*2.58)+'px';
  bathHint.textContent='[E] Usar banheiro';
  furni.appendChild(bathHint);
  // Spawn perto do elevador
...
function entrarAndar(floorNum){
  ...
  // ── FIX 4: para qualquer música anterior antes de entrar em qualquer andar ──
  ...
  // ── FIX 4: toca música SOMENTE no andar 1 ──
  // música removida
  ...
  const fw2=document.getElementById('floor-world');const fvW2=fw2?fw2.offsetWidth:window.innerWidth;
  // jogadorXAndar definido por cada buildFloor individualmente
  const fp=document.getElementById('floor-player');
  if(fp){
    fp.style.left=jogadorXAndar+'px';
    fp.style.overflow='visible'; // item filho pode aparecer fora dos limites
    fp.classList.remove('flip');
    fp.onclick=null; // só o item é clicável
    // Remove floor-item antigo e recria limpo
  ...
  // floor-item posicionado como filho do floor-player via CSS
  requestAnimationFrame(loopAndar);
}


// ── Sai do andar de volta ao elevador ──
function sairAndar(){
...
// ── Desenha o diagrama AFD no canvas do relatório ──
function desenharDiagramaAFD(){
  ...
  // Layout dos estados em posições fixas
  const nos={
    'PARADO_TERREO':  {x:210, y:290, label:'PARADO\nTÉRREO',  cor:'#ff4488'},
    'PORTAS_ABERTAS': {x:90,  y:200, label:'PORTAS\nABERTAS', cor:'#44ddff'},
    'PORTAS_FECHANDO':{x:330, y:200, label:'PORTAS\nFECHANDO',cor:'#ffaa00'},
    'SUBINDO':        {x:210, y:130, label:'SUBINDO',          cor:'#88ff66'},
    'DESCENDO':       {x:90,  y:60,  label:'DESCENDO',         cor:'#88ff66'},
    'CHEGOU':         {x:330, y:60,  label:'CHEGOU',           cor:'#ffee44'},
    'FLOOR_1':        {x:50,  y:290, label:'ANDAR 1',          cor:'#ff9fd6'},
    'FLOOR_2':        {x:370, y:290, label:'ANDAR 2',          cor:'#ff9fd6'},
    'FLOOR_3':        {x:210, y:30,  label:'ANDAR 3',          cor:'#ff9fd6'},
  };
  ...
  arestas.forEach(a=>{
    ...
    const ativa=estadoAtualElev===a.de;
...
function voltarSelecao(){
  ...
  setTimeout(()=>{document.getElementById('hud').style.display='none';document.getElementById('viewport').classList.remove('active');document.getElementById('ctrl-hint').style.display='none';document.getElementById('floor-scene').classList.remove('show');andarAberto=false;estadoAtual='LOBBY_IDLE';logAfd=[];caminhoAfd=['LOBBY_IDLE'];visitados=new Set(['LOBBY_IDLE']);t0=new Date();logAfdElev=[];caminhoAfdElev=['PARADO_TERREO'];visitadosElev=new Set(['PARADO_TERREO']);estadoAtualElev='PARADO_TERREO';andarAtualElev=0;elevadorEmMovimento=false;contadorDoces=0;itemNaMao=null;jogadorX=0;teclas={};teclasAndar={};modalAberto=false;olhandoDireita=true;document.getElementById('door-l').classList.remove('open');document.getElementById('door-r').classList.remove('open');document.getElementById('elev-num').textContent='T';document.getElementById('elev-num').style.color='#ff2222';document.getElementById('elev-interior').classList.remove('show');document.getElementById('elev-travel').classList.remove('show');document.getElementById('screen-start').style.display='flex';iniciarStartScreen();ov.classList.remove('show');},900);
}
...
function abrirMenuBar(){
  ...
  // Reutiliza o modal do restaurante com título diferente
...
function chamarGarcom(itemName, itemImg){
  ...
  // Limpa bandeja mas mantém o glow div
...
function pegarItemGarcom(){
  ...
  const src=_itemGarcom.img||'';
  const name=_itemGarcom.name;
  itensStand['waiter']=src;
  itemNaMao='waiter';

  // Garante que #floor-item existe como filho do #floor-player
  const fp=document.getElementById('floor-player');
...
function usarBanheiro(){
  ...
  overlay.innerHTML=`
    <div style="background:linear-gradient(180deg,#140030,#0a001a);border:3px solid #c77dff;border-radius:12px;padding:32px 48px;text-align:center;font-family:'Press Start 2P',monospace;box-shadow:0 0 40px rgba(199,125,255,.4);">
      ...
      <div id="bath-progress" style="width:200px;height:8px;background:rgba(255,255,255,.15);border-radius:4px;margin:10px auto 0;overflow:hidden;">
...
const GYM_SPECIAL=[
  {id:'bebedouro',  src:'IMG/Bebedouro-acdm.png', x:99,  w:0.09, label:'Bebedouro',   anim:'Bebendo água...', type:'drink'},
  {id:'vestiario',  src:'IMG/Vestiario-acmd.png', x:99,  w:0.22, label:'Vestiário',   anim:'',                type:'locker'},
];
...
function construirAndar2(furni){
  ...
  if(wall){
    ...
    // Faixas horizontais de academia
  ...
  function mkHint(id,txt,leftPx,bottomPx){
    const h=document.createElement('div');
    h.id=id;
    h.className='hint-jogo';h.style.bottom=bottomPx+'px';h.style.left=leftPx+'px';
    h.textContent=txt;
    return h;
  }

  // ── ESPELHO — do chão ao teto, cobre TODOS os equipamentos ──
  const mirrorW = Math.round(vw * 1.62); // cobre toda a seção de equipamentos
  const wallAreaH = Math.round((vh - 44) * 0.82); // chão até o teto útil
  const mirrorTop = 0; // começa do topo (teto)

  // Wrapper com overflow:hidden para conter o reflexo
  const mirrorWrap = document.createElement('div');
  mirrorWrap.style.cssText = `
    position:absolute;
    top:${mirrorTop}px;
    left:${Math.round(vw*0.02)}px;
    width:${mirrorW}px;
    height:${wallAreaH}px;
    z-index:1;
    pointer-events:none;
  ...
  // Painel principal do espelho
  const mirror = document.createElement('div');
  mirror.style.cssText = `
    position:absolute;inset:0;
    background:linear-gradient(
      108deg,
      rgba(220,235,255,.13) 0%,
      rgba(180,200,255,.07) 35%,
      rgba(200,220,255,.11) 60%,
      rgba(160,190,255,.05) 100%
    );
    border-right:3px solid rgba(200,215,255,.4);
    border-left:3px solid rgba(200,215,255,.2);
  `;

  // Reflexo diagonal brilhante (faixa de luz)
  const refl1 = document.createElement('div');
  refl1.style.cssText = `
    position:absolute;top:0;left:8%;width:10%;height:100%;
    background:linear-gradient(90deg,transparent,rgba(255,255,255,.09),transparent);
    transform:skewX(-6deg);
  `;
  // Segunda faixa menor
  const refl2 = document.createElement('div');
  refl2.style.cssText = `
    position:absolute;top:0;left:28%;width:4%;height:100%;
    background:linear-gradient(90deg,transparent,rgba(255,255,255,.05),transparent);
    transform:skewX(-6deg);
  `;
  // Borda interna (moldura interna do espelho)
  ...
  moldura.style.cssText = `
    ...
    pointer-events:none;
  `;
  // Linha de divisão central horizontal (detalhe de moldura)
  ...
  mirror.appendChild(refl1);
  mirror.appendChild(refl2);
  mirror.appendChild(moldura);
  mirror.appendChild(linha);
  ...
  GYM_EQUIP.forEach(eq=>{
    ...
    const hint=mkHint('gym-hint-'+eq.id,'[E] Treinar: '+eq.label,Math.round(vw*eq.x),fgridH+Math.round((vh-44)*0.12));
    furni.appendChild(hint);
  });

  // GYM_SPECIAL criados diretamente no scroll direito

  // Posiciona elevador
  const elevWrap=document.getElementById('floor-elev-wrap');
  if(elevWrap) elevWrap.style.left=(vw*1.88)+'px';

  // ══ SCROLL DIREITO: VESTIÁRIO → TERRAÇO ══

  // ── Mundo mais largo para caber tudo ──
  ...
  // ── SEÇÃO INTERNA (parede do vestiário) ──
  // começa depois do elevador
  const indoorStart = Math.round(vw*2.22);
  const indoorW     = Math.round(vw*0.55);

  // Parede interna — mesma cor da academia
  ...
  indoorWall.appendChild(iStripe);
  // rodapé vermelho
  ...
  indoorWall.appendChild(iRodape);
  ...
  furni.appendChild(mkHint('gym-hint-bebedouro','[E] Beber água', _bebX, fh+Math.round((vh-44)*0.12)));
  ...
  furni.appendChild(mkHint('gym-hint-vestiario','[E] Usar: Vestiário', _vestX, fh+Math.round((vh-44)*0.12)));

  // ── DECORAÇÕES DA PAREDE INTERNA ──
  ...
  [0.04, 0.60].forEach(pct=>{
    ...
    wr.style.cssText='position:absolute;top:0;left:10%;width:20%;height:100%;background:linear-gradient(90deg,rgba(255,255,255,.06),transparent);';
    const wcH=document.createElement('div');
    wcH.style.cssText=`position:absolute;top:48%;left:0;right:0;height:2px;background:rgba(60,30,160,.5);`;
    const wcV=document.createElement('div');
    wcV.style.cssText=`position:absolute;top:0;bottom:0;left:50%;width:2px;background:rgba(60,30,160,.5);transform:translateX(-50%);`;
  ...
  // ── PAREDE DE DIVISÃO (fim do indoor / início do terraço) ──
  ...
  // ── TERRAÇO (exterior) — começa após a parede de divisão ──
  const terraceStart = dividerX + 6;
  ...
  // Estrelas — distribuição uniforme por grade com offset pseudo-aleatório
  ...
  for(let row=0;row<rows;row++){
    for(let col=0;col<cols;col++){
      ...
      const sx=col*cellW+jx+2;
      const sy=row*cellH+jy+2;
      const bright=(0.4+((si*79)%55)/100);
      const sz=si%9===0?2:1;
      const s=document.createElement('div');
      s.className='estrela';s.style.left=sx+'px';s.style.top=sy+'px';s.style.width=sz+'px';s.style.height=sz+'px';s.style.background='rgba(255,255,255,'+bright.toFixed(2)+')';
      sky.appendChild(s);
    }
  }
  // Lua crescente
  ...
  moonCut.className='lua-crescente-corte';moonCut.style.background='#080222';
  ...
  deck.style.cssText=`position:absolute;bottom:${fh}px;left:${terraceStart}px;width:${terraceW}px;height:${deckH}px;background:repeating-linear-gradient(90deg,#7a4e2d 0px,#7a4e2d 17px,#6a3e1e 17px,#6a3e1e 19px);border-top:2px solid #9a6840;z-index:5;pointer-events:none;`;
  furni.appendChild(deck);

  // Corrimão frontal
  const railH=Math.round((vh-44)*0.16);
  const railBot=fh+deckH;
  // postes nas pontas
  ...
  // Piscina — centralizada no terraço
  ...
  const poolBot = fh + deckH + Math.round((vh-44)*0.02);
  ...
  function animatePool(){
    ...
    const ctx=cv.getContext('2d'),t=Date.now()/1400;
    const g=ctx.createLinearGradient(0,0,0,cv.height);
    g.addColorStop(0,'#0d4f8a');g.addColorStop(.5,'#0a3a6e');g.addColorStop(1,'#061e40');
    ...
    for(let w=0;w<3;w++){
      ctx.beginPath();
      for(let x=0;x<=cv.width;x+=2){const y=4+w*8+Math.sin(x/cv.width*Math.PI*6+t+w*1.2)*3;x===0?ctx.moveTo(x,y):ctx.lineTo(x,y);}
      ctx.stroke();
    }
    const rf=ctx.createLinearGradient(cv.width*.55,0,cv.width*.85,0);
    rf.addColorStop(0,'transparent');rf.addColorStop(.5,'rgba(200,225,255,.1)');rf.addColorStop(1,'transparent');
    ctx.fillStyle=rf;ctx.fillRect(0,0,cv.width,cv.height*.3);
  ...
  // Elevador no terraço
  const elevWrap2=document.getElementById('floor-elev-wrap');
  if(elevWrap2) elevWrap2.style.left=(vw*1.88)+'px';
  // Spawn perto do elevador
...
// ── Hints da academia atualizados a cada frame ──
function atualizarHintsAcademia(vw){
  // Equipamentos normais — pela tabela GYM_EQUIP
  GYM_EQUIP.forEach(eq=>{
    const h=document.getElementById('gym-hint-'+eq.id);
    if(!h)return;
    const cx=Math.round(vw*eq.x)+Math.round(vw*eq.w/2);
    h.style.display=Math.abs(jogadorXAndar-cx)<Math.round(vw*0.10)?'block':'none';
  });
  // Bebedouro e vestiário — usa variáveis globais de posição
  const bHint=document.getElementById('gym-hint-bebedouro');
  if(bHint) bHint.style.display=(GYM_BEB_X>0&&Math.abs(jogadorXAndar-(GYM_BEB_X+GYM_BEB_W/2))<Math.round(vw*0.14))?'block':'none';
  const vHint=document.getElementById('gym-hint-vestiario');
  if(vHint) vHint.style.display=(GYM_VEST_X>0&&Math.abs(jogadorXAndar-(GYM_VEST_X+GYM_VEST_W/2))<Math.round(vw*0.18))?'block':'none';
  // Piscina
  const poolHint=document.getElementById('gym-hint-pool');
  const poolCv2=document.getElementById('gym-pool-canvas');
  if(poolHint&&poolCv2){
    const pL=parseInt(poolCv2.style.left)||0;
    const pW=poolCv2.offsetWidth||300;
    poolHint.style.display=(jogadorXAndar>pL-60&&jogadorXAndar<pL+pW+60)?'block':'none';
  }
}
...
function interagirAcademia(vw){
  ...
  // Bebedouro
  if(GYM_BEB_X>0){
    const bebCx=GYM_BEB_X+GYM_BEB_W/2;
    if(Math.abs(jogadorXAndar-bebCx)<range){gymDrinkWater();return;}
  }
  // Vestiário
  if(GYM_VEST_X>0){
    const vestCx=GYM_VEST_X+GYM_VEST_W/2;
    if(Math.abs(jogadorXAndar-vestCx)<range+60){gymOpenLocker();return;}
  }
  // Piscina
...
function gymSwim(){
  ...
  // Move o personagem para dentro da piscina
  ...
  // Desce o personagem para dentro da água (metade do corpo)
  const origBottom=fp.style.bottom||'';
  fp.style.bottom=(poolBot2+Math.round(poolCv.offsetHeight*0.45))+'px';
  fp.style.position='absolute';

  // Animação de nadar — balança o personagem
  let swimT=0;
...
function gymOpenLocker(){
  ...
  menuVestiarioAberto=true;modalAberto=true;
  const ov=document.createElement('div');
  ov.id='locker-modal';
  ov.className='overlay-fade';
  const box=document.createElement('div');
...
function gymCloseLocker(){
  const el=document.getElementById('locker-modal');
  if(el)el.remove();
...
function construirAndar3(furni){
  furni.innerHTML='';
  const vw=window.innerWidth, vh=window.innerHeight;
  // Mundo = 1 tela — elevador + 3 portas espaçadas
  ...
  const fg=document.getElementById('floor-fgrid');
  if(fg){
    fg.style.background='repeating-linear-gradient(90deg,#0e0620 0px,#0e0620 59px,#160930 59px,#160930 60px)';
    fg.style.borderTop='3px solid rgba(199,125,255,.35)';
  }

  // Luminárias no teto — 2, uma em cada metade
  ...
  // Número do andar
  const sign=document.createElement('div');
  sign.style.cssText=`position:absolute;top:${Math.round(wallH*0.04)}px;left:50%;transform:translateX(-50%);font-family:'Press Start 2P',monospace;font-size:clamp(5px,.75vw,9px);color:rgba(199,125,255,.3);letter-spacing:5px;z-index:3;pointer-events:none;`;
  sign.textContent='3º ANDAR';
  furni.appendChild(sign);

  // ── PORTAS espaçadas — elevador à esquerda, portas em 3 posições ──
  const doorW=Math.round(vw*0.14);
  APT_DOOR_W=doorW;

  // Distribui as 3 portas uniformemente entre 30% e 95% do mundo
  ...
  doorDefs.forEach((d,i)=>{
    ...
    if(d.num===302){
      ...
      const arrow=document.createElement('div');
      arrow.style.cssText=`position:absolute;bottom:${fgridH+Math.round(wallH*0.76)}px;left:${lx+Math.round(doorW*.5)}px;transform:translateX(-50%);font-size:clamp(10px,1.5vw,18px);color:#c77dff;z-index:6;pointer-events:none;text-shadow:0 0 10px #c77dff;animation:neonPulse 0.9s ease-in-out infinite;`;
      arrow.textContent='▼';
      furni.appendChild(arrow);
    }
...
// ── Hints do corredor / apartamento 302 ──
...
function interagirAndar3(vw){
  ...
  [303,302,301].forEach(num=>{
    ...
    if(Math.abs(jogadorXAndar-cx)<Math.round(vw*0.12)){
      acted=true;
      if(num===302){_dentroApt302=true;entrarApt302();} else {baterPorta(num);}
    }
  });
...
function baterPorta(num){
  ...
  tocarTom(220,.08);setTimeout(()=>tocarTom(220,.06),120);
  notificar('Apto '+num+' — Sem resposta...');
}

// ── Entrar no apartamento 302 ──
// ── Apartamento 302 — usa floor-scene igual aos outros andares ──

// ═══════════════════════════════════════
// APARTAMENTO 302
// ═══════════════════════════════════════
function entrarApt302(){
  // Marca que está dentro do apartamento 302
  _dentroApt302=true;
  const furni=document.getElementById('floor-furni');
  // Reconstrói o interior do apto
  if(furni) construirApt302(furni);
  document.getElementById('floor-hud-name').textContent='Apartamento 302';
  // Sincroniza posição visual do personagem (spawn definido dentro de construirApt302)
  const fp=document.getElementById('floor-player');
...
// ── Monta o interior do apartamento 302 com PNGs ──
function construirApt302(furni){
  ...
  // Decoração pequena (não interativa)
  function deco(src,lx,w,z=3){
    const e=document.createElement('img');
    e.src=src;
    e.style.cssText=`position:absolute;bottom:${fgridH}px;left:${lx}px;width:${w}px;height:auto;image-rendering:pixelated;z-index:${z};pointer-events:none;`;
    e.onerror=()=>e.style.display='none';
    return e;
  }
  function mkHint(id,txt,lx){
    const h=document.createElement('div');
  ...
  // Luminária pendente centralizada em cada cômodo
  function mkLamp(cx, color='#e8c070'){
    const lamp=document.createElement('div');
  ...
  const larguras=[sacW,salaW,cozW,jantarW,quartoW,banhoW];
  const ids=['sacada','sala','cozinha','jantar','quarto','banho'];

  const pos={};
  let xAcum=0;
  ids.forEach((id,i)=>{
    pos[id]={x:xAcum,w:larguras[i]};
    const wall=document.createElement('div');
    wall.style.cssText=`position:absolute;top:0;left:${xAcum}px;width:${larguras[i]}px;height:${wallH}px;background:${bgs[id]};z-index:0;`;
    furni.appendChild(wall);
    // Friso de teto em cada cômodo
    if(id!=='sacada'){
      const friso=document.createElement('div');
      friso.style.cssText=`position:absolute;top:${Math.round(wallH*0.08)}px;left:${xAcum}px;width:${larguras[i]}px;height:4px;background:rgba(0,0,0,.08);z-index:1;pointer-events:none;`;
      furni.appendChild(friso);
      // Friso de rodapé
      const frisoB=document.createElement('div');
      frisoB.style.cssText=`position:absolute;bottom:${fgridH}px;left:${xAcum}px;width:${larguras[i]}px;height:8px;background:rgba(0,0,0,.1);z-index:1;pointer-events:none;`;
      furni.appendChild(frisoB);
      // Luminária centralizada
      furni.appendChild(mkLamp(xAcum+Math.round(larguras[i]/2)));
    }
    xAcum+=larguras[i];
  });
  ...
  // Pilares entre cômodos — sutil sombra
  const pilarW=Math.round(vw*0.006);
  ids.slice(1).forEach(id=>{
    const p=document.createElement('div');
    p.style.cssText=`position:absolute;top:0;left:${pos[id].x-pilarW}px;width:${pilarW*2}px;height:${wallH}px;background:linear-gradient(90deg,rgba(0,0,0,.18),transparent,rgba(0,0,0,.08));z-index:5;pointer-events:none;`;
    furni.appendChild(p);
  });
  ...
  predData.forEach(p=>{
    ...
    for(let r=1;r<nRows;r++) for(let col=0;col<nCols;col++){
      const seed=r*13+col*7+p.x*100;
      if((seed%3)===0) continue;
      const jan=document.createElement('div');
      const cor=(seed%5===0)?'#ffe066':(seed%4===0)?'#88ccff':'#ffaa44';
      jan.style.cssText=`position:absolute;left:${Math.round(col*(pw/nCols)+2)}px;bottom:${Math.round(r*(ph/nRows)+2)}px;width:${Math.max(3,Math.round(pw/nCols)-3)}px;height:${Math.max(4,Math.round(ph/nRows)-4)}px;background:${cor};opacity:${.6+((seed%4)*.1)};`;
  ...
  const luaCorte=document.createElement('div');
  luaCorte.style.cssText=`position:absolute;top:-10%;left:18%;width:${luaSz}px;height:${luaSz}px;border-radius:50%;background:#000510;`;
  lua.appendChild(luaCorte);
  ...
  const cores=['#ff4466','#ffcc00','#44ddff','#ff88ee','#88ff66','#ff9900'];
  const nLuz=Math.floor(sacW/18);
  for(let i=0;i<nLuz;i++){
    const lz=document.createElement('div');
    const cor=cores[i%cores.length];
    const delay=(i*0.15).toFixed(2);
    const lzX=sacX+Math.round(i*(sacW/nLuz)+6);
    lz.style.cssText=`position:absolute;top:${fioY}px;left:${lzX}px;width:7px;height:10px;background:${cor};border-radius:2px 2px 50% 50%;box-shadow:0 0 8px ${cor},0 0 16px ${cor}88;z-index:5;pointer-events:none;animation:neonPulse ${(.7+i%3*.3).toFixed(1)}s ease-in-out ${delay}s infinite;`;
    furni.appendChild(lz);
  }
  ...
  // Corrimão de vidro — no chão, appendado ANTES do balanço
  const railH=Math.round(wallH*.18);
  // Corrimão começa ACIMA do deck (bottom = fgridH = topo do deck)
  ...
  // Balanço — appendado DEPOIS do corrimão, fica na frente
  const balacoW=Math.round(sacW*0.55);
  const balacoX=sacX+Math.round((sacW-balacoW)/2);
  const balacoImg=gi('IMG/Balaço.png', balacoX, balacoW, 6);
  balacoImg.id='apt-balaco';
  furni.appendChild(balacoImg);
  furni.appendChild(mkHint('apt-h-balaco','[E] Balançar 🌙', balacoX));
  ...
  // ══ COZINHA ══
  const {x:cozX}=pos.cozinha;
  furni.appendChild(gi('IMG/cozinha-apt.png', cozX+pad, cozW-pad*2, 3));
  furni.appendChild(mkHint('apt-h-cozinha','[E] Cozinhar 🍳', cozX+pad));

  // ══ JANTAR ══
  const {x:jX}=pos.jantar;
  furni.appendChild(gi('IMG/sala-de-jantar-apt.png', jX+pad, jantarW-pad*2, 3));
  furni.appendChild(mkHint('apt-h-jantar','[E] Jantar 🍽️', jX+pad));

  // ══ QUARTO ══
  const {x:qX}=pos.quarto;
  furni.appendChild(gi('IMG/quarto-apt.png', qX+pad, quartoW-pad*2, 3));
  furni.appendChild(mkHint('apt-h-quarto','[E] Dormir 💤', qX+pad));

  // ══ BANHEIRO ══
  const {x:bX}=pos.banho;
  furni.appendChild(gi('IMG/banho-apt.png', bX+pad, banhoW-pad*2, 3));
  furni.appendChild(mkHint('apt-h-banho','[E] Tomar banho 🚿', bX+pad));

  window._dadosApt={
    exit:   {x:portaX-20, w:portaW+40},
    balaco: {x:balacoX,   w:balacoW},
    sofa:   {x:salaX,     w:salaImgW},
    cozinha:{x:cozX,      w:cozW},
    jantar: {x:jX,        w:jantarW},
    quarto: {x:qX,        w:quartoW},
    banho:  {x:bX,        w:banhoW},
  };

  // Spawn ao lado esquerdo da porta
...
function hintsApt302(vw){
  ...
  const mapa=[
    ['exit','apt-h-exit'],['balaco','apt-h-balaco'],['sofa','apt-h-sofa'],
    ['cozinha','apt-h-cozinha'],['jantar','apt-h-jantar'],['quarto','apt-h-quarto'],['banho','apt-h-banho'],
  ];
  mapa.forEach(([id,hid])=>{
...
// Interact do apt — cada item só ativa quando o jogador está dentro do seu cômodo
function interagirApt302(vw){
  if(!window._dadosApt) return;
  const d=window._dadosApt;
  // Range menor e específico: metade da largura do item
  function perto(item){ return Math.abs(jogadorXAndar-(item.x+item.w/2)) < item.w/2 + 30; }

  // Balanço — verifica primeiro, dentro da sacada
  if(perto(d.balaco)){
    const sw=document.getElementById('apt-balaco');
    if(sw){
      let a=0,dr=1;
      const anim=setInterval(()=>{a+=dr*3;if(Math.abs(a)>20)dr*=-1;sw.style.transform=`rotate(${a}deg)`;sw.style.transformOrigin='top center';},40);
...
// ── Sai do apt 302 e volta ao corredor ──
...
function mostrarSelecao(){
  ...
  // Desenha city bg na tela de seleção
  const sc=document.getElementById('select-city-bg');
  if(sc){sc.width=window.innerWidth;sc.height=window.innerHeight;desenharJanelaCidade('select-city-bg',99);}
}
...
function iniciarStartScreen(){
  ...
  // Estrelas
  for(let i=0;i<200;i++){
    const x=(i*137+11)%w, y=(i*97+17)%(h*.7);
    const r=i%8===0?.8:i%4===0?.5:.3;
    ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);
    ctx.fillStyle=`rgba(255,255,255,${.2+((i*79)%60)/100})`;ctx.fill();
  }

  // Silhueta de prédios
  const predH=Math.round(h*.28);
  const predDefs=[
    {x:.02,w:.06,h:.6},{x:.07,w:.04,h:.4},{x:.10,w:.08,h:.75},
    {x:.17,w:.05,h:.5},{x:.21,w:.09,h:.85},{x:.29,w:.04,h:.35},
    {x:.32,w:.06,h:.65},{x:.37,w:.03,h:.30},
    // direita
    {x:.62,w:.03,h:.35},{x:.64,w:.07,h:.70},{x:.70,w:.04,h:.45},
    {x:.73,w:.08,h:.80},{x:.80,w:.05,h:.55},{x:.84,w:.06,h:.40},
    {x:.89,w:.09,h:.72},{x:.97,w:.04,h:.38},
  ];
  predDefs.forEach(p=>{
    const px=Math.round(w*p.x), pw=Math.round(w*p.w), ph=Math.round(predH*p.h);
    const py=h-Math.round(h*.08)-ph;
    ...
    for(let r=1;r<rows;r++) for(let col=0;col<cols;col++){
      ...
      const cor=seed%5===0?'#ffe066':seed%4===0?'#88ccff':'#ffaa44';
      ctx.fillStyle=cor;
      ctx.globalAlpha=.5+((seed%4)*.1);
      ctx.fillRect(px+Math.round(col*(pw/cols)+1),py+Math.round(r*(ph/rows)+1),Math.max(2,Math.round(pw/cols)-2),Math.max(3,Math.round(ph/rows)-3));
    }
    ctx.globalAlpha=1;
  });
  ...
  ctx.fillRect(0,h-Math.round(h*.08),w,Math.round(h*.08));
  // Borda neon
  ctx.strokeStyle='rgba(199,125,255,.5)';
  ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(0,h-Math.round(h*.08));ctx.lineTo(w,h-Math.round(h*.08));ctx.stroke();
}

// Inicia start screen ao carregar
window.addEventListener('load',()=>{
  iniciarStartScreen();
});
